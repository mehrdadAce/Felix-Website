angular.module('umbraco').controller('DamageToolController',
    function ($scope, $location, appState, eventsService) {
        ...
        function loadOverview () {
            $.ajax({
                url: '/umbraco/backoffice/DamageTool/GetOverviewData?id=' + $scope.acquisitionId,
                dataType: 'json',
                type:"GET",
                success: function (data) {
                    ...
                    $scope.overviewModel.ChassisNumberUrl += resizeConfig;
                    ...
                    $scope.overviewModel.SchemePictures.map(x => x += resizeConfig);
        ...
        function loadSchemeOverview() {
            $.ajax({
                url: "/umbraco/backoffice/DamageTool/GetSchemeOverview?userId=" + $scope.acquisitionId,
                type: "GET",
                success: function (data) {
                    $("#scheme-in-overview").html(data);
                    $("map[name='image-map']").remove();
                }
            })
        }
        ...
        $scope.downloadFiles = function () {
            ...
            window.location = `/umbraco/backoffice/AcquisitionTool/DownloadAcquisitionFiles?id=${$scope.acquisitionId}&isTakeOver=false`;
        };
    });
